...
- `lib/game-logic.ts` for word validation, scoring, ranks, and pangram detection.
- `lib/hint-system.ts` for the five hint generators and their edge cases.
- `lib/game-state.ts` for difficulty modes, hint earning, and persistent state.
- `scripts/build_dictionary.py` for the dictionary build pipeline.
- `lib/__tests__/` for the 80-test suite. The tests are the best way to see the rules treated as real product logic instead of UI decoration.
